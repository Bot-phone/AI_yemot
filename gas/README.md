...
## השבתת הסקריפט (מתג כיבוי)

כדי להשבית את הסקריפט בלי לפרוס מחדש, הגדירו ב-**Project Settings → Script Properties** את המאפיין `AI_YEMOT_DISABLED` עם טקסט כלשהו (למשל `תחזוקה עד יום ראשון`). הערך עצמו מוצג למשתמש כסיבה. ערך ריק, `0` או `false` משאירים את הסקריפט פעיל.

כשהמאפיין מוגדר, כל בקשה (כולל התראות מנהל) מקבלת את הסימן המוסכם:

```json
{ "status": "disabled", "code": "SCRIPT_DISABLED", "message": "<הסיבה>" }
```

תוכנת שולחן העבודה מזהה את הסימן הזה (`ai.rs` → `SCRIPT_DISABLED: `) ומציגה הודעה ייעודית "הסקריפט המתאם הושבת על ידי המפעיל" עם הסיבה, במקום שגיאת תקשורת כללית. מחיקת המאפיין מחזירה את הסקריפט לפעולה מיד.
