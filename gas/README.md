...
> **מצב "סקריפט" הוא כיום נתיב תאימות (legacy).**
> אפליקציית הדסקטופ מריצה את מצב "ישיר" (direct) בלולאת סוכן מקומית — קריאת מסמכי ידע וקובצי ini מתבצעת במחשב המשתמש, ללא מעבר דרך ה-Apps Script וללא שליחת טוקן.
> הסקריפט כאן נשמר עבור משתמשים ותצורות ישנות, ולכן חוזה הפלט שלו (שורות `הסבר:` + כתובות `UpdateExtension`, ושורות `הבהרה:`) חייב להישאר יציב.
